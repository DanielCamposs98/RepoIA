    ...
    protected void evaluateTerms() {
        ...
        for (int i = 0; i < terms.length; i++) {
            if (terms[i] == null) // Null function => Nothing to do
                ...
                if (var.isOutput())
                    throw new RuntimeException("Can't use an output variable '" + var.getName() + "' for a function (It may create a race condition)");
            ...
            if (debug)
                Gpr.debug("Term[" + i + "]: " + terms[i] + "\tClass:" + terms[i].getClass().getSimpleName() + "\tValue: " + values[i]);
        }
    }
