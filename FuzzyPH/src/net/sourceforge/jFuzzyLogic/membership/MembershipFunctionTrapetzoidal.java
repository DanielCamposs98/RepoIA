...
public class MembershipFunctionTrapetzoidal extends MembershipFunctionContinuous {
    ...
    public boolean checkParamters(StringBuffer errors) {
        ...
        if (parameters[0].getValue() > parameters[1].getValue()) {
            ok = false;
            if (errors != null)
                errors.append("Parameter midLow is out of range (should stisfy: min <= midLow): " + parameters[0] + " > " + parameters[1] + "\n");
        }

        if (parameters[1].getValue() > parameters[2].getValue()) {
            ok = false;
            if (errors != null)
                errors.append("Parameter midHigh is out of range (should stisfy: midLow <= midHigh): " + parameters[1] + " > " + parameters[2] + "\n");
        }

        if (parameters[2].getValue() > parameters[3].getValue()) {
            ok = false;
            if (errors != null)
                errors.append("Parameter max is out of range (should stisfy: midHigh <= max): " + parameters[2] + " > " + parameters[3] + "\n");
        }
    ...
    public double membership(double in) {
        ...
        if (in < parameters[1].getValue())
            return ((in - parameters[0].getValue()) / (parameters[1].getValue() - parameters[0].getValue()));
