...
public class MembershipFunctionPieceWiseLinear extends MembershipFunctionContinuous {
    ...
    public boolean checkParamters(StringBuffer errors) {
        ...
        if (x.length > 1) for (int i = 0; i < x.length; i++) {
            if ((i > 0) && (x[i - 1].getValue() > x[i].getValue())) {
                ok = false;
                if (errors != null)
                    errors.append("Array not sorted: x[" + (i - 1) + "] = " + x[i - 1] + " , x[" + i + "] = " + x[i] + "\n");
            }

            if ((y[i].getValue() < 0) || (y[i].getValue() > 1)) {
                ok = false;
                if (errors != null)
                    errors.append("Membership funcion out of range: y[" + i + "] = " + y[i] + " (should be in range [0,1]\n");
            }
        }
    ...
    public double membership(double in) {
        ...
        for (i = 1; i < len; i++)
            if (in <= x[i].getValue())
                return y[i - 1].getValue() + (y[i].getValue() - y[i - 1].getValue()) * ((in - x[i - 1].getValue()) / (x[i].getValue() - x[i - 1].getValue()));
