...
public class MembershipFunctionTriangular extends MembershipFunctionContinuous {
    ...
    public boolean checkParamters(StringBuffer errors) {
        ...
        if (parameters[0].getValue() > parameters[1].getValue()) {
            ok = false;
            if (errors != null)
                errors.append("Parameter mid is out of range (should stisfy: min <= mid): " + parameters[0] + " > " + parameters[1] + "\n");
        }

        if (parameters[1].getValue() > parameters[2].getValue()) {
            ok = false;
            if (errors != null)
                errors.append("Parameter max is out of range (should stisfy: mid <= max): " + parameters[1] + " > " + parameters[2] + "\n");
        }
