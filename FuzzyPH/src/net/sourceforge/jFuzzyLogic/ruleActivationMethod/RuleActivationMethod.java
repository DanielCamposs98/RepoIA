    ...
    public void imply(RuleTerm fuzzyRuleTerm, RuleAccumulationMethod ruleAccumulationMethod, double degreeOfSupport) {
        ...
        if (mf.isDiscrete()) {
            ...
            if (Double.isNaN(x) || Double.isInfinite(x))
                throw new RuntimeException("Universe minimum not calculated for term '" + fuzzyRuleTerm.getTermName() + "' : " + x);
            if (Double.isNaN(step) || Double.isInfinite(step))
                throw new RuntimeException("Step not calculated for term '" + fuzzyRuleTerm.getTermName() + "' : " + step);
    ...
    public String toStringCpp(RuleTerm fuzzyRuleTerm, RuleAccumulationMethod ruleAccumulationMethod, String degreeOfSupportName) {
        ...
        if (fuzzyRuleTerm.getMembershipFunction().isDiscrete()) {
            ...
            if (fuzzyRuleTerm.isNegated()) out.append("\t\tmembership = 1 - mf.membership(x);\n");
                ...
                out.append("\t\t\tdouble membership = " + fuzzyRuleTerm.getLinguisticTerm().toStringCppMethodName(fuzzyRuleTerm.getVariable()) + "(x);\n");
