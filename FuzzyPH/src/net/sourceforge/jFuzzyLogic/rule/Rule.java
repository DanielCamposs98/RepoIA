...
public class Rule extends FclObject implements CompileCpp {
    ...
    public Rule addAntecedent(Variable variable, String termName, boolean negated) {
        if (variable.getMembershipFunction(termName) == null)
            throw new RuntimeException("RuleTerm '" + termName + "' does not exists in variable '" + variable.getName() + "'");
    ...
    public void evaluate() {
        ...
        if (debug)
            Gpr.debug("degreeOfSupport:" + degreeOfSupport + "\truleAccumulationMethod:" + ruleAccumulationMethod + "\truleImplicationMethod:" + ruleActivationMethod);
