...
import net.sourceforge.jFuzzyLogic.ruleAccumulationMethod.*;
...
public class RuleBlock extends FclObject implements Iterable<Rule>, Comparable<RuleBlock>, CompileCpp {
    ...
    RuleAccumulationMethod createAccumulationMethod(String ruleAccumulationMethodType) {
        ...
        if (ruleAccumulationMethodType.equalsIgnoreCase("MAX"))
            ruleAccumulationMethod = new RuleAccumulationMethodMax();
        else if (ruleAccumulationMethodType.equalsIgnoreCase("BSUM"))
            ruleAccumulationMethod = new RuleAccumulationMethodBoundedSum();
        else if (ruleAccumulationMethodType.equalsIgnoreCase("NSUM"))
            ruleAccumulationMethod = new RuleAccumulationMethodNormedSum();
        else if (ruleAccumulationMethodType.equalsIgnoreCase("PROBOR"))
            ruleAccumulationMethod = new RuleAccumulationMethodProbOr();
        else if (ruleAccumulationMethodType.equalsIgnoreCase("SUM"))
            ruleAccumulationMethod = new RuleAccumulationMethodSum();
        else
            throw new RuntimeException("Unknown/Unimplemented Rule accumulation method '" + ruleAccumulationMethodType + "'");
    ...
    private void fclTreeRuleBlockRule(Tree tree, RuleConnectionMethod and, RuleConnectionMethod or) {
        ...
        for (int childNum = 1; childNum < tree.getChildCount(); childNum++) {
            ...
            if (type.equalsIgnoreCase("IF"))
                fuzzyRule.setAntecedents(fclTreeRuleBlockRuleIf(child.getChild(0), and, or));
            ...
            for (RuleTerm term : llc) {
                ...
                if (resetted.get(var) == null) {
                    ...
                    if (var.getDefuzzifier() == null)
                        throw new RuntimeException("Defuzzifier not set for output variable '" + var.getName() + "'");
    ...
    public String toStringFcl() {
        ...
        for (Rule rule : rules) {
            ...
            rb.append("\tRULE " + name + " : " + rule.toStringFcl() + "\n");
            if (rule.getAntecedents().getRuleConnectionMethod() != null)
                operator = rule.getAntecedents().getRuleConnectionMethod().toStringFcl();
            ruleNum++;
        }
