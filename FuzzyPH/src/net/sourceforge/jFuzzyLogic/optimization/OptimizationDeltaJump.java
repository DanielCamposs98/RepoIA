...
public class OptimizationDeltaJump extends OptimizationMethod {
    ...
    private double jump(int parameterNumber, double error0) {
        ...
        if (param.set(paramValue + epsilon)) {
            ...
            System.out.println("Cannot set parameter [" + parameterNumber + "] to " + (paramValue + epsilon));
    ...
    public void optimizeIteration(int iterationNum) {
        ...
        for (paramNum = 0; paramNum < len; paramNum++) {
            ...
            if (verbose)
                System.out.println("Parameter [" + paramNum + "]: " + parameter.getName() + "\tvalue: " + paramValue);
            ...
            if (jump != 0) { // Can jump?
                ...
                for (lineIterNum = 0; lineIterNum < maxLineSearchIterations; lineIterNum++)
                    ...
                    if (parameter.set(paramValue + alpha * jump)) { // Can set parameter?
                        ...
                        if (verbose)
                            System.out.println("\t\tLine iteration:" + lineIterNum + ", alpha:" + alpha + ", Delta_Error:" + deltaErr);
                ...
                if (verbose) {
                    if (lineIterNum >= maxLineSearchIterations)
                        System.out.println("\t\tWARNING: Too many line iterations! This may point a desing error in your fuzzy system.");
                    if (paramValue != parameter.get())
                        System.out.println("\tIteration: " + iterationNum + "\tParameter: " + paramNum + " (" + parameter.getName() + ")\tError before: " + error0 + "\tError after: " + bestAlphaError + "\tParameter before: " + paramValue + "\t after: " + parameter.get() + "\tbest alpha: " + bestAlpha);
                }
            }
        }
