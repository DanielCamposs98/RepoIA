...
public class OptimizationPartialDerivate extends OptimizationMethod {
    ...
    private double derivate(int parameterNumber, double error0) {
        ...
        else if (param.set(paramValue - epsilon))
            epsilon *= -1; // Cant' increase? => Decrease parameter (in this dimention) to estimate derivate
        ...
        if (verbose)
            System.out.println("Parameter:" + param.getName() + "\tDerivate: " + derivate + "\tepsilon: " + epsilon);
    ...
    public void optimizeIteration(int iterationNum) {
        ...
        for (int paramNum = 0; paramNum < len; paramNum++) {
            ...
            if (verbose)
                System.out.println("Iteration: " + iterationNum + "\tParameter: " + paramNum + "\tError: " + error0);
            ...
            if (Math.abs(derivate) <= EPSILON) {
                ...
                if (verbose)
                    System.out.println("Derivate's abs too small (derivate: " + derivate + " < " + parameterList.get(paramNum).getEpsilon() + ")");
            } else {
                //---
                ...
                for (int lineIterNum = 0; lineIterNum < maxLineSearchIterations; lineIterNum++) {
                    ...
                    if (verbose)
                        System.out.println("\tLine iteration: " + lineIterNum + "\talpha: " + alpha + "\tDelta_Error: " + (error - error0));
                    if (error < error0) {
